...
export async function runLoopOnce(job: JobRecord, deps: LoopRunnerDeps): Promise<LoopRunnerResult> {
  ...
  /**
   * One reviewable markdown file per cleared brief, written under
   * `docs/aeo-briefs/<brief_id>.md` in the customer repo. The MCP
   * `open_pr` tool's live mode (see `mcp/src/tools/index.ts`) will
   * clone the repo into a temp dir, write these files, commit, push,
   * and then open the PR. We deliberately do NOT auto-apply
   * `brief.diff_patch` yet — the brief lands in the repo as a human-
   * reviewable changelog so the customer's reviewer is the one who
   * decides whether to merge any concrete edits.
   */
  const briefFiles = cleared.map(({ brief, gap, page_url }) => ({
    path: `docs/aeo-briefs/${brief.brief_id}.md`,
    content: renderBriefMarkdown(brief, gap, page_url, headlineAudit.audit_id),
  }));

  const dryRun = job.payload["dry_run"] === true;
  if (dryRun) {
    emit("dry_run", {
      brief_id: headBrief.brief_id,
      branch,
      files: briefFiles.length,
      reason: "payload.dry_run=true; skipping open_pr",
    });
    emit("done", { pr_number: 0, pr_url: "(dry-run)", conflicts_detected, sublations_recorded });
    return {
      audit_id: headlineAudit.audit_id,
  ...
  emit("open_pr", { brief_id: headBrief.brief_id, branch, files: briefFiles.length });
...
/**
 * Render a single brief as a self-contained, reviewable markdown file.
 * This is what gets committed into `docs/aeo-briefs/<brief_id>.md` of
 * the customer repo. The reviewer reads this in the GitHub diff and
 * decides whether to act on it; we deliberately don't auto-edit
 * customer pages.
 */
function renderBriefMarkdown(
  ...
  return [
    `# AEO brief — ${gap.tactic}`,
    ``,
    `- **brief_id**: \`${brief.brief_id}\``,
    `- **gap_id**: \`${gap.gap_id}\``,
    `- **page**: ${page_url}`,
    `- **evidence_tier**: ${gap.evidence_tier}`,
    `- **predicted_lift_pp**: +${gap.predicted_lift_pp}pp`,
    `- **geo_paper_reference**: ${gap.geo_paper_reference}`,
    `- **pre_audit_id**: ${audit_id}`,
    `- **created_at**: ${brief.created_at}`,
    `- **claude_model**: ${brief.claude_model}`,
    ``,
    `## Rationale`,
    ``,
    brief.rationale_md.trim() || "_(no rationale supplied)_",
    ``,
    `## Suggested change (diff hint)`,
    ``,
    "```diff",
    (brief.diff_patch || "").trim() || "(no diff supplied — manual edit recommended)",
    "```",
    ``,
    `## Revert plan`,
    ``,
    brief.revert_plan_md.trim() || "_(no revert plan supplied)_",
    ``,
    `## Provenance`,
    ``,
    `Generated by llm-seo-lab AEO loop. This file is the audit trail for the`,
    `recommendation; merging this PR records consent. The recommendation is`,
    `subject to Pratyakṣa sublation in future loop runs — a stronger`,
    `recommendation will reference this brief\\'s id.`,
    ``,
  ].join("\n");
}
